...
def test_get_service(services_client, rmock, service_json):
    ...
    result = services_client.get_service(service_json['id'])

    assert rmock.called
    assert result == service_json


def test_get_services(services_client, rmock, services_json):
    ...
    result = services_client.get_services()

    assert rmock.called
    assert result == services_json


def test_get_service_template(services_client, rmock, service_template_json):
    ...
    result = services_client.get_service_template(
...
def test_get_service_templates(services_client, rmock, service_templates_json):
    ...
    result = services_client.get_service_templates(
        service_templates_json[0]['service'])
